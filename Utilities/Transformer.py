...
class DecisionTransformer(nn.Module):
    def __init__(self, state_dim, act_dim, n_blocks, h_dim, context_len, 
        ...
        # # discrete actions
        self.embed_action = torch.nn.Embedding(max_timestep, h_dim) # not act_dim
        ...
        #self.predict_rtg = torch.nn.Linear(h_dim, act_dim)
        self.predict_rtg = nn.Sequential(
                                nn.Linear(h_dim, 4*h_dim),
                                nn.GELU(),
                                nn.Linear(4*h_dim, act_dim),
                            )
    ...
    def forward(self, timesteps, states, actions, returns_to_go):
        ...
        time_embeddings = self.embed_timestep(timesteps).to(dtype=torch.float32)

        #print(time_embeddings.dtype,states.dtype)
        
        # time embeddings are treated similar to positional embeddings
        state_pre_embed = self.embed_state(states)
        #print(state_pre_embed.dtype)
        state_embeddings = state_pre_embed+ time_embeddings
        #print(actions.shape)
